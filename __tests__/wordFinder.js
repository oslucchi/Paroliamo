...
function findWordsInMatrix(matrix, trie, { minWordLen = 4, diagonals = true } = {}) {
  ...
  function dfs(r, c, current, path) {
    ...
    if (nextWord.length >= minWordLen && trie.hasWord(nextWord)) {
      const key = nextWord + ":" + path.map(([pr, pc]) => `${pr},${pc}`).join("|");
...
/*
const { trie } = loadDictionary("./words.txt", { minLength: 1 });
...
const found = findWordsInMatrix(matrix, trie, { minWordLen: 4, diagonals: true });
console.log(found);
// -> [ { word: 'rope', path: [[0,3],[1,3],[1,2],[1,1]] }, ... ]
*/
